...
case class ParsedLine(from: Int, to: Int, letter: Char, password: String) {
  def isValid: Boolean = {
    val count = getLetterCount
    (count >= from) && (count <= to)
  }
  def isValidPart2: Boolean = {
    val hasFirst = password(from - 1) == letter
    val hasSecond = password(to - 1) == letter
    hasSecond != hasFirst
  }
  def getLetterCount: Int = {
    password.foldLeft(0)((count, char) => if(char == letter) count + 1 else count)
  }
}
object ParsedLine {
  val regexPattern = "(\\d+)-(\\d+) ([a-z]): ([a-z]+)".r
  def apply(string: String): ParsedLine = {
    regexPattern.findFirstMatchIn(string) match {
      case Some(i) => new ParsedLine(i.group(1).toInt, i.group(2).toInt, i.group(3)(0), i.group(4))
    }
  }
}
object Password {
  val input = """5-6 s: zssmssbsms
                |3-6 j: jjjjjrrj
                |4-7 k: kfkgkkkkk
                |2-3 n: nkbgfnn
                ...
                |1-7 m: mmmmmmvm
                |4-5 r: rrrrv
                |6-7 c: xkdzcscg""".stripMargin

  def main(args: Array[String]): Unit = {
    val parsedLines = input.split("\n").map(ParsedLine(_))
    println("Part I", parsedLines.map(_.isValid).count(a => a))
    println("Part II", parsedLines.map(_.isValidPart2).count(a => a))
  }
